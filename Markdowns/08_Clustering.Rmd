...
```{r leiden_1c, fig.height=6, fig.width=8}
plotReducedDim(sce, 
               dimred = "TSNE_corrected",
               colour_by = "leiden20", 
               text_by = "leiden20")
...
some additional inter-connectedness between other clusters. 
...
of k, this can take a long time to run. Usually I do not run this interactively
in R studio, but instead write an R script that will end by exporting the final
output of clusterSweep to an RDS object which I can later load into R. This R
script can then be sent as a job to the cluster. This is also means the job can
be massively more parallelized by using more cores. We will see this in the
...
plotReducedDim(sce, 
               ...
               colour_by = "label", 
               text_by = "label") +
  ggtitle("Walktrap k=15 clusters")
```
